...
SELECT percentile_disc(0.5) within group (order by(p.price * least("output_matchedFillResults"->'left'->'makerFeePaid', "output_matchedFillResults"->'right'->'takerFeePaid')::numeric / 10^token_mat.decimals)) as usd
